...
$fwrite(f,"----------------------------------------------------------------\n");   
$fwrite(f,"-- Testcase 2 - BNADDV\n");
$fwrite(f,"----------------------------------------------------------------\n");

// Write IMEM from File
write_imem_from_file_tl_ul(.log_filehandle(f), .imem_file_path({mem_path, "imem_bnaddv_test.txt"}), .clk(clk_i), .clk_cycles(cc), .start_address(0), .tl_o(tl_o), .tl_i(tl_i_d) );
...
write_dmem_from_file_tl_ul(.log_filehandle(f), .dmem_file_path({mem_path, "dmem_bnaddv_test.txt"}), .clk(clk_i), .clk_cycles(cc), .start_address(0), .tl_o(tl_o), .tl_i(tl_i_d) );
...
for (int i=0 ; i<64 ; i++) begin 
    ...
    case(i)
      ...
      16   :  assert (rdbk == 32'hfffffffe) else begin $fwrite(f,"Wrong Result! Act: %d\n", rdbk); error_count ++; end
      17   :  assert (rdbk == 32'hfffffffd) else begin $fwrite(f,"Wrong Result! Act: %d\n", rdbk); error_count ++; end
      18  :   assert (rdbk == 32'hfffffffc) else begin $fwrite(f,"Wrong Result! Act: %d\n", rdbk); error_count ++; end
      19  :   assert (rdbk == 32'hfffffffb) else begin $fwrite(f,"Wrong Result! Act: %d\n", rdbk); error_count ++; end
      20  :   assert (rdbk == 32'hfffffffa) else begin $fwrite(f,"Wrong Result! Act: %d\n", rdbk); error_count ++; end
      21  :   assert (rdbk == 32'hfffffff9) else begin $fwrite(f,"Wrong Result! Act: %d\n", rdbk); error_count ++; end
      22  :   assert (rdbk == 32'hfffffff8) else begin $fwrite(f,"Wrong Result! Act: %d\n", rdbk); error_count ++; end
      23  :   assert (rdbk == 32'hfffffff7) else begin $fwrite(f,"Wrong Result! Act: %d\n", rdbk); error_count ++; end
      ...
      48   :  assert (rdbk == 32'hfffdfffe) else begin $fwrite(f,"Wrong Result! Act: %d\n", rdbk); error_count ++; end
      49   :  assert (rdbk == 32'hfffbfffc) else begin $fwrite(f,"Wrong Result! Act: %d\n", rdbk); error_count ++; end
      50  :   assert (rdbk == 32'hfff9fffa) else begin $fwrite(f,"Wrong Result! Act: %d\n", rdbk); error_count ++; end
      51  :   assert (rdbk == 32'hfff7fff8) else begin $fwrite(f,"Wrong Result! Act: %d\n", rdbk); error_count ++; end
      52  :   assert (rdbk == 32'hfff5fff6) else begin $fwrite(f,"Wrong Result! Act: %d\n", rdbk); error_count ++; end
      53  :   assert (rdbk == 32'hfff3fff4) else begin $fwrite(f,"Wrong Result! Act: %d\n", rdbk); error_count ++; end
      54  :   assert (rdbk == 32'hfff1fff2) else begin $fwrite(f,"Wrong Result! Act: %d\n", rdbk); error_count ++; end
      55  :   assert (rdbk == 32'hffeffff0) else begin $fwrite(f,"Wrong Result! Act: %d\n", rdbk); error_count ++; end
